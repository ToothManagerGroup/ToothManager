...
- (void)setModel:(TTMApointmentModel *)model {
    ...
    switch (model.status) {
        ...
        case TTMApointmentStatusStarting: {
            ...
            NSDate *startTime = [NSDate fs_dateFromString:model.start_time format:@"yyyy-MM-dd HH:mm:ss"];
            NSTimeInterval time = -[startTime timeIntervalSinceNow] / 60;
            self.datetimeLabel.text = [NSString stringWithFormat:@"已用时 %@", [[NSString stringwithNumber:@(time)] hourMinutesTimeFormat]];
            break;
        }
